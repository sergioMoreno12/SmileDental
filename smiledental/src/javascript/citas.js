...
dniEm.addEventListener("blur", function() {
    if (dniEm.value !== "") {
    ...
    if (!validarDNI(dniEm.value) && dniEm.value !== "") {
        dniEm.value = "";
...
horaCita.addEventListener("change", function(){
    ...
    if(horaCita.value < 13){
     ...
     for(var i = 0; i < nombreDr.length; i++){
        nombreDr[i].style.display="block";
     }
    }
    else if(horaCita.value >16){
...
tipo.addEventListener("change", function(){
    ...
    switch(tipo.value){
        case "cirugia":
         ...
         default: //puede ser prevent-default.
