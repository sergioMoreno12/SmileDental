...
if(!isset($_SESSION['username'])){
    ...
    if ($_SERVER['REQUEST_METHOD'] === "GET" && isset($_GET['fecha'], $_GET['hora'])){
          ...
          $hora = $_GET['hora'];
          $citas = $cita->revisaCitas($fechaSolicitada,$hora);
          $ids = $dentist->getIdGeneral();
       ...
       if(!$cita->tieneCitaGeneral($id_paciente) || $hoy > $fechaSolicitada || $fechaUltimaCita < $hoy){
        // si no aparece en la tabla o si la fecha de hoy es menor a la de la cita que pidio o si la ultima cita es menor a la fecha de hoy
        foreach($ids as $dentista_id){
            var_dump($ultimaCita);
            if(!in_array($dentista_id, $citas)){
                $dentista_disponible = $dentista_id;
                break;
            }}
            if($dentista_disponible !== null){
                $cita->insertCita($id_paciente, $dentista_disponible, $fechaSolicitada, $hora);
 ...
 <div class="row align-items-center">
    ...
    <div class ="col-sm-6 col-md-9 text-center d-block "> 
     ...
     <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
        ...
        <input type="submit" value="Volver" class="bg-light p-1 mt-1">
     </form>
    </div>
 </div>
</header>
        ...
        <table class="table" id="altas">
            ...
            <tbody class="border border-dark">
                <?php
                    ...
                    echo "<td><a href ='profile.php?Borrar=".$citas['id_cita']."'>Borrar Cita</a><p>Una vez borrada la cita no se puede recuperar</p>\n";
                    echo"</tr>\n";
...
xhr.open("GET", "../Ajax/getHoras.php?fecha=" + this.value, true);
xhr.send();
